...
# from airflow.m import dag, task
import logging
...
# FEED_CONFIG_HOME = "/usr/local/airflow/dags/dds_feed_configs/"FEED_CONFIG_HOME = Variable.get("FEED_CONFIG_HOME")
...
def check_for_feed_list(COMPLETE_LIST):
    return_value='run_dds_task_groups'
    if len(COMPLETE_LIST)==0:
        return_value='trigger_summary_report'
    return return_value
...
ts5 = BashOperator(
        task_id='run_dds_task_groups',
        bash_command = 'echo "run dds task group"',
        dag=dag)

# ts6 = BashOperator(
#         task_id='run_summary_report',
#         bash_command = 'echo "run summary report"',
#         dag=dag)
...
ts7 = TriggerDagRunOperator(
        task_id="trigger_summary_report",
        trigger_dag_id="consolidated_dds_summary_report",
        dag=dag,
...
ts8 = DummyOperator( # dummy operator for ending
        task_id='end',
        dag=dag)
...
COMPLETE_LIST_TMP = FULL_LOAD_IND_LIST + INCR_LOAD_IND_LIST + PARTITION_LOAD_IND_LIST + DEPENDENT_LIST_FINAL
...
ts4 = BranchPythonOperator(
        task_id='check_for_feed_list',
        python_callable=check_for_feed_list,
        op_args=[COMPLETE_LIST],
        dag=dag)
