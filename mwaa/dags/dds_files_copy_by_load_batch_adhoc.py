...
# SUCCESS_EMAIL = ["mahesh.nyalam@advance-auto.com"]
# FAILURE_EMAIL = ["mahesh.nyalam@advance-auto.com"]
summary_email_dl = "mahesh.nyalam@advance-auto.com"
# summary_email_dl = Variable.get("summary_email_dl")
# ADHOC_DDS_LIST = json.loads(Variable.get('ADHOC_DDS_DICT',deserialize_json=True))["FEEDNAMES"]
...
def empty_the_adhoc_list():
    x={"FEEDNAMES":["DUMMY_FEED"]}
    Variable.update(key='ADHOC_DDS_DICT', value=json.dumps(x))
...
ts4 = ShortCircuitOperator(
        task_id='empty_the_adhoc_list',
        python_callable = empty_the_adhoc_list,
...
feed_list = ADHOC_DDS_LIST
# feed_list = ['ITEMXREF','ALL_STORES']
...
FULL_LOAD_IND_LIST = [x for x in feed_list if x in feed_load_types_list.keys() and feed_load_types_list[x] == ['FULL_LOAD'] and x in feed_dependencies['INDEPENDENT']] # INDEPENDENT FULL LOAD list
INCR_LOAD_IND_LIST = [x for x in feed_list if x in feed_load_types_list.keys() and feed_load_types_list[x] == ['INCR_LOAD_WITHOUT_PARTITION'] and x in feed_dependencies['INDEPENDENT']] # INDEPENDENT INCR_LOAD_WITHOUT_PARTITION list
PARTITION_LOAD_IND_LIST = [x for x in feed_list if x in feed_load_types_list.keys() and feed_load_types_list[x] == ['INCR_LOAD_WITH_PARTITION'] and x in feed_dependencies['INDEPENDENT']] # INDEPENDENT INCR_LOAD_WITH_PARTITION list
...
MASTER_LIST_FINAL = list(set(feed_list) & set(MASTER_LIST))
DEPENDENT_LIST_FINAL = list(set(feed_list) & set(DEPENDENT_LIST_FINAL1))
INDEPENDENT_LIST = FULL_LOAD_IND_LIST + INCR_LOAD_IND_LIST + PARTITION_LOAD_IND_LIST
# COMPLETE_LIST = FULL_LOAD_IND_LIST + INCR_LOAD_IND_LIST + PARTITION_LOAD_IND_LIST + DEPENDENT_LIST_FINAL + MASTER_LIST_FINAL
if len(MASTER_LIST_FINAL) == 0:
    COMPLETE_LIST = FULL_LOAD_IND_LIST + INCR_LOAD_IND_LIST + PARTITION_LOAD_IND_LIST + DEPENDENT_LIST_FINAL
else:
    COMPLETE_LIST = INDEPENDENT_LIST

COMPLETE_LIST_SMALL = [item for item in COMPLETE_LIST if item not in HEAVY_LOAD_FEEDS]
COMPLETE_LIST_LARGE = [item for item in COMPLETE_LIST if item in HEAVY_LOAD_FEEDS]

if len(COMPLETE_LIST_SMALL) == 0:
    COMPLETE_LIST_SMALL.append('DUMMY_FEED')
ind_tasks_dict = dict()
...
TaskGroupsListSmall =[]
TaskGroupsListLarge=[]
com_lt_small = [list(item) for item in list(zip_longest(*[iter(COMPLETE_LIST_SMALL)]*7, fillvalue=None))]
com_lt_small = [ list(filter(None, _)) for _ in com_lt_small ]

for g_id in enumerate(com_lt_small, 1):
    tg_id = 'SMALL_DDS_GROUP_{}'.format(g_id[0])
    if g_id[0]==1:
    ...
    with TaskGroup(group_id=tg_id,dag=dag) as taskgroup1:
        ...
        for feedname in g_id[1]:
            ...
            elif feedname in DEPENDENT_LIST_FINAL:
                load_type = feed_load_types_list[feedname][0]
...
if len(COMPLETE_LIST_LARGE)!=0:
    com_lt_large = [list(item) for item in list(zip_longest(*[iter(COMPLETE_LIST_LARGE)]*2, fillvalue=None))]
    com_lt_large = [ list(filter(None, _)) for _ in com_lt_large ]
    for g_id in enumerate(com_lt_large, 1):
        tg_id = 'LARGE_DDS_GROUP_{}'.format(g_id[0])
        if g_id[0]==1:
            trigger_condition="all_success"
        else:
            trigger_condition="all_done"
...
if len(MASTER_LIST_FINAL) != 0:
    with TaskGroup(group_id="DEPENDENT_DDS_GROUP",dag=dag) as taskgroups2:
        for feedname in MASTER_LIST_FINAL:
            load_type = feed_load_types_list[feedname][0]
            sm1 = PythonOperator(
                task_id='s3copy_job_for_{}'.format(feedname),
                python_callable = main,
                op_args=[env, feedname, load_type, batchnum, summary_email_dl,"DEPENDENT_DDS_GROUP","{{ task_instance.xcom_pull(task_ids='get_cycle_load_date') }}"],
                execution_timeout=pendulum.duration(minutes=90),
                on_failure_callback=failure_function_wrapper('s3copy_job_for_{}'.format(feedname)),
                dag=dag)
            
            master_dict_stg ={feedname: sm1}
            master_tasks_dict.update(master_dict_stg)

        for feedname in DEPENDENT_LIST_FINAL:
            load_type = feed_load_types_list[feedname][0]
            sd1 = PythonOperator(
                    task_id='s3copy_job_for_{}'.format(feedname),
                    python_callable = main,
            ...
            upstream_tasks =[]
            for feed_name in feed_dependencies['DEPENDENT']['SUB_LIST_LEVEL1'].keys():
                #loop through dependent feeds in each master list 
                for i in feed_dependencies['DEPENDENT']['SUB_LIST_LEVEL1'][feed_name]:
                    if i == feedname:
                        upstream_master_task = master_tasks_dict[feed_name]
                        upstream_tasks.append(upstream_master_task)
                    else:
                        continue
