...
if len(COMPLETE_LIST) == 0:
    COMPLETE_LIST.append('DUMMY_FEED')
# ind_tasks_dict = dict()
...
com_lt = [list(item) for item in list(zip_longest(*[iter(COMPLETE_LIST)]*7, fillvalue=None))]
com_lt = [ list(filter(None, _)) for _ in com_lt ]

for g_id in enumerate(com_lt, 1):
    tg_id = 'DDS_GROUP_{}'.format(g_id[0])
    if g_id[0]==1:
        trigger_condition="all_success"
    else:
        trigger_condition="all_done"
    task_group_name = str(tg_id)
    with TaskGroup(group_id=tg_id,dag=dag) as taskgroups1:
