...
MASTER_LIST_TMP = [x for x in feed_dependencies['DEPENDENT']['SUB_LIST_LEVEL1'].keys()] # DEPENDENT list
DEPENDENT_LIST = []
...
DEPENDENT_LIST_TMP = list(set(feed_list) & set(DEPENDENT_LIST_FINAL1))
INDEPENDENT_LIST_TMP = FULL_LOAD_IND_LIST + INCR_LOAD_IND_LIST + PARTITION_LOAD_IND_LIST
INDEPENDENT_LIST = [item for item in INDEPENDENT_LIST_TMP if item not in ADHOC_DDS_LIST]
INDEPENDENT_LIST_SMALL = [item for item in INDEPENDENT_LIST if item not in HEAVY_LOAD_FEEDS]
INDEPENDENT_LIST_LARGE = [item for item in INDEPENDENT_LIST if item in HEAVY_LOAD_FEEDS]
MASTER_LIST = [item for item in MASTER_LIST_TMP if item not in ADHOC_DDS_LIST]
DEPENDENT_LIST_FINAL = [item for item in DEPENDENT_LIST_TMP if item not in ADHOC_DDS_LIST]

if len(INDEPENDENT_LIST_SMALL) == 0:
    INDEPENDENT_LIST_SMALL.append('DUMMY_FEED')
# COMPLETE_LIST = INDEPENDENT_LIST + MASTER_LIST
ind_tasks_dict = dict()
...
with TaskGroup(group_id="DEPENDENT_DDS_GROUP",dag=dag) as taskgroup3:
    for feedname in MASTER_LIST:
